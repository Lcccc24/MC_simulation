...
void PX4CtrlFSM::process()
{
	...
	case CMD_CTRL:
	{
		...
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("\033[32m[px4ctrl] CMD_CTRL(L3) --> AUTO_HOVER(L2)\033[32m");
		}
		else
		{
			des = get_cmd_des();
	...
	else //if (!param.use_position_ctrl)
	{
	...
	// lc CHANGE position control pub 
	if(param.use_position_ctrl && px4_choose_msg == 0)
	{
		publish_position_ctrl(des,now_time);
		//ROS_INFO("PX4 POSITION CONTROL");
	}

	else if(param.use_position_ctrl && px4_choose_msg == 1)
	{
		//ROS_INFO("ITE POSITION CONTROL");
	}
...
void PX4CtrlFSM::publish_position_ctrl(const Desired_State_t &des, const ros::Time &stamp)
{
	...
	//msg.type_mask = 0b100000000000; // xyz + vxvyvz + axayaz + yaw
	msg.type_mask = mavros_msgs::PositionTarget::IGNORE_YAW_RATE;

	msg.position.x = des.p.x();
	msg.position.y = des.p.y();
	msg.position.z = des.p.z();
	msg.velocity.x = des.v.x();
	msg.velocity.y = des.v.y();
	msg.velocity.z = des.v.z();
	msg.acceleration_or_force.x = des.a.x();
	msg.acceleration_or_force.y = des.a.y();
	msg.acceleration_or_force.z = des.a.z();
	msg.yaw = des.yaw;

	pos_ctrl_FCU_pub.publish(msg);
}
...
//lc add
void PX4CtrlFSM::px4_choose_callback(const std_msgs::Int32::ConstPtr& msg)
{
	px4_choose_msg = msg->data;
}