...
public:
    ...
    struct Options
    {
        ...
        /// 视觉观测参数
        // xy位置噪声
        double vision_xy_noise_ = 0.05;
        // z位置噪声
        double vision_z_noise_ = 0.1;
        // 横滚角、俯仰角噪声
...
private:
    ...
    void CalcVisionNoise(double z)
    {
        ...
        static double vrp2 = options_.vision_roll_pitch_noise_ * options_.vision_roll_pitch_noise_;
        static double vy2 = options_.vision_yaw_noise_ * options_.vision_yaw_noise_;
...
bool ESKF<S>::ObserveSE3(const SE3 &pose)
{
    ...
    Mat3T h13 = R_sc_.matrix().transpose() * SO3::hat(R_.matrix().transpose() * (p_ + dx_.template block<3, 1>(0, 0) + R_em_.matrix() * p_mt_));
    ...
    innov.template head<3>() = (pose.translation() - p_ct_);         // 平移部分
    innov.template tail<3>() = (R_ct_.inverse() * pose.so3()).log(); // 旋转部分(3.67)
