...
public:
  ...
  inline void setUniformBspline(const Eigen::MatrixXd &points, const int &order,
                                const double &interval) {
  ...
  inline Eigen::VectorXd evaluateDeBoor(const double &u) // use u \in [up, u_mp]
  {
  ...
  inline UniformBspline getDerivative() {
  ...
  parameterizeToBspline(const double &ts,
    ...
    if (point_set.size() <= 3) {
      cout << "[B-spline]:point set have only " << point_set.size()
           << " points." << endl;
      return;
    }
