...
public:
    ...
    struct Options
    {
        ...
        // uwb观测噪声
        double uwb_noise_ = 0.1;

        // 相对位置观测噪声
        double relative_position_xy_noise_ = 0.02;
        double relative_position_z_noise_ = 0.02;
    ...
    /// 使用UWB观测
    bool ObserveUWB(const double &distance);

    /// 使用里程计相对位置观测
    bool ObserveRelativePosition(const Vec3d &pos);
...
bool ESKF<S>::ObserveSE3(const SE3 &pose)
{
    ...
    Mat3T h13 = R_sc_.matrix().transpose() * SO3::hat(R_.matrix().transpose() * (p_mt_ - p_));
...
template <typename S>
bool ESKF<S>::ObserveUWB(const double &distance) {
    if (p_.norm() < 0.1 ) {
        return false;
    }

    // 观测矩阵
    Eigen::Matrix<S, 1, 18> H = Eigen::Matrix<S, 1, 18>::Zero();
    Eigen::Matrix<S, 1, 3> dh_dp = p_.transpose() / p_.norm();
    H.template block<1, 3>(0, 0) = dh_dp;
    
    // 感测噪声协方差矩阵
    Eigen::Matrix<S, 1, 1> V;
    V(0, 0) = options_.uwb_noise_;

    // 卡尔曼增益
    Eigen::Matrix<S, 18, 1> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + V).inverse();

    S innov = distance - p_.norm();
    dx_ = K * innov;    
    cov_ = (Mat18T::Identity() - K * H) * cov_;
...
//当前姿态对此相对位置观测的影响？？
template <typename S>
bool ESKF<S>::ObserveRelativePosition(const Vec3d &pos){
    ...
    // 假设XY平面和Z方向的噪声不同
    Mat3T R = Mat3T::Zero();
    R(0, 0) = options_.relative_position_xy_noise_ * options_.relative_position_xy_noise_ * (std::fabs(pos.z()));
    R(1, 1) = options_.relative_position_xy_noise_ * options_.relative_position_xy_noise_ * (std::fabs(pos.z()));
    R(2, 2) = options_.relative_position_z_noise_ * options_.relative_position_z_noise_ * (std::fabs(pos.z()));
    
    //卡尔曼增益
    Eigen::Matrix<S, 18, 3> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + R).inverse();
