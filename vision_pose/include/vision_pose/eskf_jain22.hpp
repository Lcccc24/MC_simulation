...
public:
    ...
    using MotionNoiseT = Eigen::Matrix<S, 9, 9>; // 运动噪声类型
                              ...
                              const VecT &gravity = VecT(0, 0, -9.8))
    {
        ...
        ba_ = init_ba;
        g_ = gravity;
        cov_ = Eigen::Matrix<S, 9, 9>::Identity() * 1e-4;
    }

    /// 使用IMU递推
    bool Predict(const sensor_msgs::Imu &imu, const Eigen::Quaterniond &q_em);

    /// 使用视觉观测
    bool ObserveVision(const geometry_msgs::PoseStamped &pose, const Eigen::Quaterniond &q_em);
    ...
    /// 设置协方差
    void SetCov(const Eigen::Matrix<S, 9, 9> &cov) { cov_ = cov; }
    ...
    void Reset(const Eigen::Quaterniond &q_es)
    {
        R_ = SO3(q_es);
        first_vision_ = true;
    }
...
private:
    void BuildProcessNoise(const Options &options)
    {
        ...
        // 设置过程噪声
        Q_.diagonal() << 0, 0, 0, ev2, ev2, ev2, et2, et2, et2;
    }
    ...
    void UpdateAndReset()
    {
        ...
        ProjectCov();
        dx_.template block<3, 1>(6, 0) = Vec3d::Zero();
    }

    /// 对P阵进行投影，参考式(3.63)
    void ProjectCov()
    {
        Eigen::Matrix<S, 9, 9> J = Eigen::Matrix<S, 9, 9>::Identity();
        J.template block<3, 3>(6, 6) = SO3::exp(-0.5 * dx_.template block<3, 1>(6, 0)).matrix();
    ...
    {
        double z2 = z * z;
        double vxy2 = options_.vision_xy_noise_ * options_.vision_xy_noise_ * z2;
        double vz2 = options_.vision_z_noise_ * options_.vision_z_noise_ * z2;
    ...
    /// 误差状态
    Eigen::Matrix<S, 9, 1> dx_ = Eigen::Matrix<S, 9, 1>::Zero();

    /// 协方差阵
    Eigen::Matrix<S, 9, 9> cov_ = Eigen::Matrix<S, 9, 9>::Identity();
...
template <typename S>
bool ESKF<S>::Predict(const sensor_msgs::Imu &imu, const Eigen::Quaterniond &q_em)
{
    ...
    // nominal state 递推
    VecT new_p = p_ + v_ * dt;
    VecT new_v = v_ - R_ * SO3::exp(dx_.template block<3, 1>(6, 0)) * (acce - ba_) * dt - g_ * dt;
    // SO3 new_R = R_ * SO3::exp((gyro - bg_) * dt);

    VecT new_delta = dx_.template block<3, 1>(6, 0) + ((gyro - bg_) - 0.5 * SO3::hat(gyro - bg_) * dx_.template block<3, 1>(6, 0)) * dt;
    // SO3 new_R = R_ * SO3::exp(new_delta);

    dx_.template block<3, 1>(0, 0) = new_p;
    dx_.template block<3, 1>(3, 0) = new_v;
    dx_.template block<3, 1>(6, 0) = new_delta;
    ...
    // 注意bg和ba的顺序
    Eigen::Matrix<S, 9, 9> F = Eigen::Matrix<S, 9, 9>::Identity();                      // 主对角线
    F.template block<3, 3>(0, 3) = Mat3T::Identity() * dt;                              // p 对 v
    F.template block<3, 3>(3, 6) = R_.matrix() * SO3::hat((acce - ba_)) * dt;          // v对theta
    F.template block<3, 3>(6, 6) = Mat3T::Identity() - 0.5 * SO3::hat(gyro - bg_) * dt; // theta 对 theta
...
template <typename S>
bool ESKF<S>::ObserveVision(const geometry_msgs::PoseStamped &pose, const Eigen::Quaterniond &q_em)
{
    // 判断时间戳
    ...
    if (first_vision_)
    {
        ...
        R_ = R_em_ * R_mt_ * R_ct.inverse() * R_sc_.inverse();
        p_ = R_.matrix() * p_sc_ + R_.matrix() * R_sc_.matrix() * p_ct - R_em_.matrix() * p_mt_;
...
bool ESKF<S>::ObserveSE3(const SE3 &pose)
{
    ...
    Eigen::Matrix<S, 6, 9> H = Eigen::Matrix<S, 6, 9>::Zero();
    Mat3T h11 = R_sc_.matrix().transpose() * R_.matrix().transpose();
    Mat3T h13 = R_sc_.matrix().transpose() * SO3::hat(R_.matrix().transpose() * (p_ + R_em_.matrix() * p_mt_));
    Mat3T h23 = -R_mt_.matrix().transpose() * R_em_.matrix().transpose() * R_.matrix();
    ...
    // 根据预测值计算观测值
    Vec3d p_ct_hat = R_sc_.matrix().transpose() * R_.matrix().transpose() * (p_ + R_em_.matrix() * p_mt_) - R_sc_.matrix().transpose() * p_sc_;
    SO3 R_ct_hat = R_sc_.inverse() * R_.inverse() * R_em_ * R_mt_;

    Vec3d d_theta = -R_mt_.matrix().transpose() * R_em_.matrix().transpose() * R_.matrix() * dx_.template block<3, 1>(6, 0);

    // 更新x和cov
    Vec6d innov = Vec6d::Zero();
    innov.template head<3>() = (pose.translation() - p_ct_hat);         // 平移部分
    innov.template tail<3>() = (R_ct_hat.inverse() * pose.so3()).log(); // 旋转部分(3.67)
    // innov.template tail<3>() = d_theta;                                 // 旋转部分

    dx_ = dx_ + K * innov;
    cov_ = (Eigen::Matrix<S, 9, 9>::Identity() - K * H) * cov_;
